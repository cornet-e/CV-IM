   ...
   # == Choix du numéro de lot ===
    ...
    if mode == "Simulation de 3 lots":
        ...
        if uploaded_file:
            ...
            st.warning("En attente de données. Veuillez charger un fichier ou choisir le mode simulation.")
    ...
    if df is not None:
        ...
        for lot in df['Lot'].unique():
            ...
            n, m, med, sd_class = len(subset), subset.mean(), subset.median(), subset.std()
            mad_val = stats.median_abs_deviation(subset, scale='normal')
            sd_iqr = stats.iqr(subset) / 1.349
            
            stats_list.append({
                "Lot": lot, "N": n, "Moyenne": m, "Médiane": med,
                "SD Classique": sd_class, "CV Classique (%)": (sd_class/m)*100,
                "SD MAD": mad_val, "CV MAD (%)": (mad_val/med)*100,
                "SD IQR": sd_iqr, "CV IQR (%)": (sd_iqr/med)*100
            })
